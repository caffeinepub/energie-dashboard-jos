...
actor {
  ...
  var userYearlyPrices = Map.empty<Principal, Map.Map<Nat, YearlyPrices>>();
  ...
  public shared ({ caller }) func setYearlyPrices(year : Nat, gasPrice : Float, elecPrice : Float) : async () {
    if (not (AccessControl.hasPermission(accessControlState, caller, #user))) {
  ...
  public query ({ caller }) func calculateCosts(
    ...
    defaultElectricityPricePerKwh : Float,
    year : Nat16,
  ) : async Costs {
    if (not (AccessControl.hasPermission(accessControlState, caller, #user))) {
